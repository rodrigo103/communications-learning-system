...
| ● | $\left(\dfrac{S}{N}\right)_{out}\Big|_{dB} = \left(\dfrac{S}{N}\right)_{in}\Big|_{dB} - F_T\Big|_{dB}$ | La relación que más se usa | En lineal es $(S/N)_{out} = (S/N)_{in}/F_T$. $F_T$ en dB **es** cuántos dB de SNR se pierden |
| | $T_{sys}=T_{antena}+T_{receptor}$ | Temperatura de sistema | Aditiva |
| | $B_N = \dfrac{1}{|H(f_0)|^2}\displaystyle\int_0^\infty |H(f)|^2df$ | Ancho de banda **equivalente de ruido** | Hace $N=N_0B_N$ **exacta**. Casi siempre lo dan |
...
| $\displaystyle\int|x(t)|^2dt = \int|X(f)|^2df$ | **Parseval** (conservación de energía) |
