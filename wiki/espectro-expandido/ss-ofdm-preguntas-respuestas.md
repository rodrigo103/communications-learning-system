...
## OFDM paga por todo esto con ancho de banda?

No. La cuenta lo demuestra:

$$B_T = N_p \cdot \Delta f = N_p \cdot \frac{1}{T_S} = N_p \cdot \frac{R_b}{N_p \cdot \ell} = \frac{R_b}{\ell}$$

$N_p$ se cancela. $B_T = R_b/\ell$, exactamente el mismo ancho de banda que una sola portadora usando la misma $\ell$-QAM. OFDM **empata** en ancho de banda. Lo que si paga:

| Costo | Detalle |
|---|---|
| **PAPR altisimo** | Suma coherente de subportadoras — peor caso: PAPR = $N_p$ = 36 dB |
| **Sensibilidad a error de frecuencia** | $\Delta f$ es minimo (~977 Hz); un desvio rompe ortogonalidad → ICI |
| **Overhead del CP** | Tiempo muerto (~1%), no es ancho de banda pero si throughput |
...
## Ayudame a entender la seccion: "La razon central: ISI por multitrayecto"
...
## Es la primera vez que en el programa de la materia leo sobre el problema del eco. Por que? Para otras tecnologias no es un problema?
...
| Tecnologia | Como sufre el eco | Como se arregla |
|---|---|---|
| **FM analogica** | Desvanecimiento (fading) | Efecto captura: se engancha a la señal mas fuerte |
| **AM analogica** | Fading + distorsion | No se arregla bien (TV analogica: "fantasmas") |
| **Digital una portadora** | ISI | Ecualizador adaptivo (filtro que estima y revierte el canal) |
| **DSSS** | ISI entre chips | RAKE receiver: trata cada eco como replica util y las combina |
| **OFDM** | ISI entre simbolos | Simbolos largos + prefijo ciclico: lo esquiva en vez de resolverlo |

La diferencia es que OFDM **pone el truco en el diseño de la forma de onda** en vez de en el receptor. Un ecualizador para 16 Mbps con una sola portadora es caro y dificil de adaptar; OFDM cambia la señal para que el canal parezca "plano" y la correccion sea trivial (una division compleja por subportadora). [analysis]

## 5G supuestamente es apta para aplicaciones de tiempo real. Que tecnologia usa? OFDM es 4G?
...
## Explicame la seccion: "Los beneficios adicionales"
...
**2. El prefijo ciclico sale barato.** El CP es tiempo muerto — una copia del final del simbolo para absorber el eco. Su costo es $CP/T_S$:

| Sistema | Simbolo | Una guarda de 10 μs cuesta |
|---|---|---|
| OFDM | 1024 μs | **~1%** |
| Una portadora | 0,625 μs | **1600%** — absurdo |

Solo con simbolos largos el prefijo ciclico es viable.
