...
>
> **La cuenta completa de "rearmando", sin atajos** — de donde sale que el resultado es real, si en el medio se multiplico por algo imaginario. Aplicando $H(f)=-j\,\text{sgn}(f)$ isla por isla (isla $+f_c$: $\text{sgn}=+1$; isla $-f_c$: $\text{sgn}=-1$):
> $$\hat S(f) = -j\,\text{sgn}(f)\,S(f) = \frac12\Big[\underbrace{-j\cdot(+1)}_{\text{isla }+f_c}M(f-f_c)+\underbrace{-j\cdot(-1)}_{\text{isla }-f_c}M(f+f_c)\Big] = -\frac{j}{2}M(f-f_c)+\frac{j}{2}M(f+f_c)$$
> Para chequear que esto es de verdad el par de Fourier de $m(t)\sin(2\pi f_ct)$, se calcula ese lado por separado: con $\sin(2\pi f_ct)=\frac{e^{j2\pi f_ct}-e^{-j2\pi f_ct}}{2j}$ y la propiedad de modulacion ($m(t)e^{\pm j2\pi f_ct}\leftrightarrow M(f\mp f_c)$):
> $$\mathcal{F}\{m(t)\sin(2\pi f_ct)\} = \frac{1}{2j}\big[M(f-f_c)-M(f+f_c)\big] = -\frac{j}{2}M(f-f_c)+\frac{j}{2}M(f+f_c)$$
> (usando $\frac1j=-j$, porque $j\cdot(-j)=1$). Es identico, termino a termino, a $\hat S(f)$ de arriba — por unicidad de la Transformada de Fourier, $\hat s(t)=m(t)\sin(2\pi f_ct)$. El $j$ no desaparece por arte de magia: el $-j$ que le toco a la isla $+f_c$ y el $+j$ que le toco a la isla $-f_c$ (signos opuestos, por el $\text{sgn}(f)$ opuesto de cada una) son exactamente la combinacion $-j(\cdot)+j(\cdot)$ que Euler reescribe como $\frac{1}{2j}[(\cdot)-(\cdot)]$ — la forma del seno. Los dos $j$ se cancelan entre si, no se pierden.
>
...
> **¿Toda señal tiene frecuencia bien definida? ¿Derivar la fase es la unica forma de calcularla si no es constante? ¿Como se determina la fase instantanea, y esta relacionada con la amplitud?** [analysis]
>
> *¿Hay señales sin frecuencia bien definida? Si, la mayoria*: solo un tono puro $Ae^{j2\pi f_0t}$ (o $A\cos(2\pi f_0t+\phi)$) tiene "una" frecuencia en sentido clasico: un unico numero constante. Cualquier otra señal —suma de tonos, FM, ruido, un pulso— no la tiene. Lo que si tiene es un **espectro** (cuanto hay de cada frecuencia, sumado sobre *todo* el tiempo — vista de Fourier, estatica) o, por el camino de Hilbert, una **frecuencia instantanea** $f_i(t)$ que varia con $t$ (vista dinamica, un numero distinto en cada instante). Son dos formas de mirar "frecuencia" genuinamente distintas.
>
> *¿Derivar la fase es la unica forma? Si, para asignar UN numero por instante — con una condicion importante*: dentro del marco de "quiero un unico valor de frecuencia en cada instante", $f_i(t)=\frac{1}{2\pi}\frac{d\phi(t)}{dt}$ es, en la practica, la unica definicion razonable (es casi tautologico: es la definicion). Pero esto tiene sentido fisico limpio solo cuando la señal es de "un solo componente" (angosta de banda, la misma condicion $f_c\gg W$ del teorema pasabanda — ver diagrama arriba en Aplicaciones). Si la señal tiene contenido de frecuencia genuinamente mixto (ej. $x(t)=\cos(2\pi f_1t)+\cos(2\pi f_2t)$ con $f_1,f_2$ comparables), el $f_i(t)$ calculado via Hilbert sigue dando algun numero, pero puede oscilar de forma rara entre $f_1$ y $f_2$ e incluso hacerse momentaneamente negativo cerca de los cruces por cero de la envolvente — deja de significar "la frecuencia" en el sentido intuitivo. Para esos casos la herramienta correcta es una **representacion tiempo-frecuencia completa** (espectrograma/STFT), no un unico $f_i(t)$. Mas avanzado que lo que pide el final, pero explica por que "derivar la fase" no es una receta universal sin condiciones.
>
> *¿Como se determina la fase instantanea, en la practica?*: $\phi(t)=\text{atan2}(\hat x(t),x(t))$ — usando atan2 (arcotangente de dos argumentos), no un arctan simple, porque $\arctan(\hat x/x)$ solo da valores en $(-\pi/2,\pi/2)$ y pierde el cuadrante; atan2 usa el signo de ambas componentes para ubicar el angulo correcto en $(-\pi,\pi]$. Si se quiere una fase que crezca sin limite (como en FM, donde $\phi(t)$ realmente crece indefinidamente, no da vueltas de $-\pi$ a $\pi$), hace falta ademas **desenrollado de fase** (*phase unwrapping*): sumar multiplos de $2\pi$ cada vez que la fase "salta" por el wraparound del atan2, para reconstruir la trayectoria continua antes de derivarla.
>
> *¿Fase y amplitud estan relacionadas?*: dos niveles de respuesta. (1) Como coordenadas polares de un numero complejo, son independientes por definicion — en $x_a(t)=a(t)e^{j\phi(t)}$, $a(t)=|x_a(t)|$ dice "que tan grande" es el fasor, $\phi(t)=\angle x_a(t)$ dice "hacia donde apunta", ninguna se deduce de la otra a partir de la formula sola. (2) Pero si se construye esa descomposicion a mano (se elige una envolvente $a(t)$ y una fase $\phi(t)$ por separado, armando $x(t)=a(t)\cos\phi(t)$ esperando que $\hat x(t)=a(t)\sin\phi(t)$), esa expectativa solo se cumple bajo la misma condicion de separacion espectral del teorema pasabanda, generalizada (se llama **teorema de Bedrosian**): hace falta que el espectro de $a(t)$ este claramente separado del espectro de $\cos\phi(t)$. Si no se cumple, $\hat x(t)$ deja de ser simplemente $a(t)\sin\phi(t)$ y envolvente y fase quedan acopladas, no son totalmente libres de elegir.

> **¿La formula $f_i(t)=f_c+k_fm(t)$ deja de valer con FM de banda ancha (WBFM)? ¿La materia ve WBFM?** [analysis]
>
> *Dos "banda ancha" distintas, no confundir*: **Eje 1**, la condicion del teorema pasabanda de Hilbert (la que puede romper el atajo instantaneo): si la portadora $f_c$ es mucho mayor que el ancho de banda *ocupado por la señal transmitida* (Carson: $B_T=2(\Delta f+f_m)$), es decir $f_c\gg\Delta f+f_m$. **Eje 2**, la clasificacion NBFM vs. WBFM de la propia FM: segun el indice de modulacion $\beta=\Delta f/f_m$ — $\beta\ll1$ (tipicamente $\le0{,}2$) es banda angosta (FM se comporta como AM, 2 bandas laterales significativas); $\beta$ no chico (ej. FM comercial, $\beta=5$) es banda ancha (Carson, muchas bandas laterales). **Estos dos ejes son independientes.**
>
> *La formula funciona para NBFM y WBFM por igual*: el FM comercial ($f_c\approx100$MHz, $\Delta f=75$kHz, $f_m=15$kHz, $B_T=200$kHz) es wideband en el Eje 2 ($\beta=5$) pero cumple de sobra el Eje 1 ($f_c\gg B_T$, factor ~500). El atajo $f_i(t)=f_c+k_fm(t)$ no se rompe con FM de banda ancha — se rompe solo si fallara el Eje 1, y eso practicamente nunca pasa en un sistema de radio real (las antenas necesitan $f_c$ mucho mas alta que el ancho de banda de la señal por razones de tamaño fisico, $\lambda/4$, asi que el sistema casi esta obligado a cumplir esta condicion). El caso que si rompe la formula es otra cosa: una señal sin portadora dominante (ej. el ejemplo de dos tonos comparables de mas arriba) — eso no es "FM de banda ancha", es no tener estructura de portadora+desviacion.
>
...
> **¿Que significa "sacarle la rotacion rapida de portadora"?** En forma polar, $x_a(t)=a(t)e^{j\phi(t)}$ es un fasor girando en el plano complejo: modulo $a(t)$, angulo $\phi(t)$. Para una señal pasabanda centrada en $f_c$, esa fase tiene tipicamente la forma $\phi(t)=2\pi f_ct+\theta(t)$ — un termino **rapido** ($2\pi f_ct$, crece lineal, gira a la velocidad de la portadora) mas uno **lento** ($\theta(t)$, donde vive la modulacion real: en AM seria constante, en FM/PM/QAM es el mensaje). Multiplicar por $e^{-j2\pi f_ct}$ resta exactamente el termino rapido de la fase, sin tocar el modulo (multiplicar por una exponencial compleja de modulo 1 no cambia el modulo): queda $a(t)e^{j\theta(t)}$, el fasor "sin girar". Es la misma operacion que pararse a girar a la misma velocidad que la portadora (cambiar a un sistema de referencia rotante): desde ahi el giro rapido parece quieto y solo se ve el tambaleo lento de la modulacion. En frecuencia esto es un corrimiento espectral ($\tilde X(f)=X_a(f+f_c)$, ver mas abajo) — de ahi el nombre "equivalente pasabajos": es literalmente bajar el espectro pasabanda (centrado en $f_c$) a banda base (centrado en $f=0$), el mismo calculo que hace un demodulador I/Q de SDR al multiplicar la señal recibida por $e^{-j2\pi f_ct}$. [analysis]
